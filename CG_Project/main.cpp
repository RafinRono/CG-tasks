...
// Draw a Red 1x1 Square centered at origin
    ...
    glVertex2f(0.0f, 0.0);    // x, y
    glVertex2f(600.0f, 0.0);    // x, y
    glVertex2f(0.0f, 0.0);    // x, y
    glVertex2f(0.0f, 450);    // x, y
    glVertex2f(0.0f, 0.0);    // x, y
    glVertex2f(-600, 0.0);    // x, y
...
void bottomWater()
{
    ...
    glBegin(GL_POLYGON);//green field left
    ...
    glBegin(GL_POLYGON);//green field right
    glColor3ub(4, 163, 109);
...
void rotate_1(int value)  //rotates left windmill
{
    _angle1+=2.0f;
    if(_angle1 > 360.0)
    {
...
void rotate_2(int value)  //rotates right windmill
{
    _angle2+=3.0f;
    if(_angle2 > 360.0)
    {
...
void DamSideBoatAnimation()
{
    static float boatPositionX = -400.0f; // Initial X position of the boat
    boatPositionX += 0.05f; // Increment X position to move the boat
    if (boatPositionX > 400.0f)
    {
        boatPositionX = -400.0f; // Reset the boat to the left when it reaches the right side
    }
